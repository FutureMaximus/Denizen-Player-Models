...
##Note: If you have the mccosmetics core shader file you can skip this part
...
#To use animations use the player model template provided and get the dmodel file generated by the dmodels converter afterwards put the file in "Denizen/player_models/animations/my_animation.dmodel.yml"
...
pmodels_load_event:
    ...
    events:
      after server start:
      - ~run pmodels_load_animation def.animation_name:samus_emotes
      #- ~run pmodels_load_model def.model_name:player_model_template_slim
...
pmodels_load_animation:
    ...
    script:
    - define yamlid dmodels_<[animation_name]>
    ...
    - define animations <yaml[<[animation_name]>].read[animations]||<map>>
    - yaml unload id:<[yamlid]>
    - yaml unload id:<[animation_name]>
    - foreach <[order]> as:id:
